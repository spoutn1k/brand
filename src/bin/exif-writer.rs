...
use clap::Parser;
use image::{GenericImageView, ImageEncoder, ImageReader, codecs::jpeg::JpegEncoder};
use regex::Regex;
...
fn encode_exif(e: &ExposureData) -> Result<Vec<u8>, Box<dyn Error>> {
    ...
    if let Some(off) = gps_off {
        dir.write_tag(Tag::Unknown(0x8825), off.offset)?;
    }
    ...
    if let Some(iso) = &e.iso {
        ...
        dir.write_tag(Tag::Unknown(0x8827), vec![iso].as_slice())?;
    }
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for (index, data) in exposures {
        ...
        targets
            .unwrap()
            .iter()
            .map(|file: &PathBuf| -> Result<(), Box<dyn Error>> {
                let photo = ImageReader::open(&file)?.with_guessed_format()?.decode()?;

                let buffer_name = file.with_extension("exifed");
                let buffer = File::create(&buffer_name)?;

                let jpg_encoder =
                    JpegEncoder::new_with_quality(buffer, 90).with_exif(encode_exif(&data)?);
                ...
                let (height, width) = photo.dimensions();
                match format(photo) {
                ...
                std::fs::rename(&buffer_name, file.with_extension("test"))?;
